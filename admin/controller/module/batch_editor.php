...
class ControllerModuleBatchEditor extends Controller {
	...
	private $filters = array (
		'number' => array ('price', 'quantity', 'sort_order', 'date_added', 'minimum', 'points', 'weight', 'length', 'width', 'height'),
	...
	public function index() {
		...
		$this->data['no_image'] = $this->model_tool_image->resize('no_image.jpg', 40, 53);
	...
	public function getProducts() {
		...
		$this->data['sort'] = (isset ($this->request->post['sort'])) ? (string) $this->request->post['sort'] : 'p.date_added';
		
		if (!in_array ($this->data['sort'], $sorts)) {
			$this->data['sort'] = 'p.date_added';
		}
		
		$this->data['order'] = (isset ($this->request->post['order'])) ? (string) $this->request->post['order'] : 'DESC';
		
		$this->data['date_added'] = (isset ($this->request->post['date_added'])) ? (string) $this->request->post['date_added'] : 'p.date_added DESC';
			...
			foreach ($this->data['setting']['fields'] as $name=>$field) {
				if ($name == 'image') {
					...
					if ($result['image'] && file_exists (DIR_IMAGE . $result['image'])) {
						$this->data['products'][$i]['thumb'] = $this->model_tool_image->resize($result['image'], 100, 133);
					} else {
	...
	public function getProductList() {
		...
		if (isset ($this->links[$path])) {
			...
			$this->data['product_image'] = $this->model_tool_image->resize($this->model_catalog_batch_editor_get_lists->getProductImage((int) $this->data['product_id']), 40, 53);
	...
	public function setting() {
		...
		$this->data['fields_default'] = array ('model', 'sku', 'upc', 'location', 'quantity', 'stock_status_id', 'image', 'manufacturer_id', 'shipping', 'price', 'points', 'tax_class_id', 'date_available', 'weight', 'weight_class_id', 'length', 'width', 'height', 'length_class_id', 'subtract', 'minimum', 'sort_order', 'status', 'viewed', 'date_added');
		...
		$exclude_fields = array ('product_id');
			...
			if ($results) {
				...
				foreach ($results as $result) {
					if (!is_dir ($result)) {
						...
						$images[] = array (
							'file' => $file,
							'img'  => $this->model_tool_image->resize($dir . $file, 40, 53)
						);
					}
