...
class ControllerToolImport extends Controller { 
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			if ((isset( $this->request->files['upload'] )) && (is_uploaded_file($this->request->files['upload']['tmp_name']))) {
				$file = $this->request->files['upload']['tmp_name'];
               	if ($this->model_tool_import->upload($file)===true) {
					$this->session->data['success'] = $this->language->get('text_success');
					$this->redirect($this->url->link('tool/import', 'token=' . $this->session->data['token'], 'SSL'));
				}
				else {
	...
	public function settings() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')) {
			...
			$this->model_setting_setting->editSetting('import', $this->request->post);
	...
	protected function getForm() {
		...
		$this->data['import'] = $this->url->link('tool/import/upload', 'token=' . $this->session->data['token'], 'SSL');
		$this->data['settings'] = $this->url->link('tool/import/settings', 'token=' . $this->session->data['token'], 'SSL');
